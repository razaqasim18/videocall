<div>
    ...
    <div class="space-y-8">
        <!-- Header Section -->
        <x-header-section headerwprimary="Packages" headerwsecondary="List"
            tagline="Review and manage your purchased packages efficiently." />
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        ...
                        <th class="px-6 py-4 font-semibold">Package Name</th>
                    ...
                    @forelse ($packages as $package)
                        <tr class="transition-all hover:bg-primary/[0.03] group">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium  text-indigo-700 bg-indigo-100/50 border border-indigo-200 ">
                                    {{ Str::ucfirst($package->status) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('agent.packages.purchase', ['id' => $package->id]) }}"
                                        wire:navigate
                                    ...
                                    @if ($package->status != 'approved')
                                        <button onclick="openDeleteModal({{ $package->id }})"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <h3 class="text-lg font-bold text-dark">No packages Found</h3>
    ...
    <div wire:ignore>
        <x-delete-modal id="deleteModal" title="Delete purchase request"
            message="Are you sure you want to delete this purchase request?" confirmAction="delete"
            closeAction="closeDeleteModal" />
    </div>

    <script>
        function openDeleteModal(packageId) {
            // 1. Tell Livewire which package is being deleted
            @this.set('packageIdToDelete', packageId);
        ...
        function closeDeleteModal() {
            ...
            @this.set('packageIdToDelete', null);
        }
