  @auth('agent')
      ...
      <div x-data="{ open: @js(request()->routeIs('agent.packages.*')) }" class="relative">
          ...
          <div x-show="open" x-cloak x-transition class="pl-4 mt-2 space-y-1">
              <a href="{{ route('agent.packages.purchase') }}" wire:navigate
                  class="flex items-center gap-3 px-4 py-2 rounded-lg text-sm transition-all
                        {{ request()->routeIs('agent.packages.purchase') ? 'text-primary font-bold bg-primary/10' : 'text-dark/60 hover:text-primary hover:bg-gray-100' }}">
                  <span
                      class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('agent.packages.purchase') ? 'bg-primary' : 'bg-gray-300' }}"></span>
                  View Packages
              </a>
              <a href="{{ route('agent.packages.report') }}" wire:navigate
                  ...
                  <span
                      class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('agent.packages.report') ? 'bg-primary' : 'bg-gray-300' }}"></span>
                  Report Packages
              </a>
          </div>
      </div>
